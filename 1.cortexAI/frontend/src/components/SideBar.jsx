...
function SideBar() {
  ...
  const handleCreateConversation = async () => {
    try {
      ...
      // Safely unwrap data depending on backend response shape
      const newConvo = data?.conversation || data?.data || data;

      if (newConvo) {
        dispatch(addConversation(newConvo));
        dispatch(setSelectedConversation(newConvo));
      }
    } catch (error) {
      ...
      <div className='hidden lg:flex flex-col items-center w-[56px] h-screen bg-[#0d0f14] border-r border-white/[0.06] py-4 gap-1 shrink-0'>
        ...
        <div className='flex-1 overflow-y-auto px-2.5 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pt-5'>
          {conversations?.map((conv, i) => {
            const isActive = Boolean(selectedConversation?._id && conv?._id && selectedConversation._id === conv._id);
            return (
              <div
                key={conv?._id || i}
